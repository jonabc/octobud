...
func TestSQLiteScheduler_EnqueueProcessNotification_Persisted(t *testing.T) {
	...
	var count int
	err = testDB.QueryRowContext(ctx, "SELECT COUNT(*) FROM jobs WHERE queue = ?", QueueProcessNotification).
		Scan(&count)
	require.NoError(t, err)
	...
	// Verify payload
	var payload string
	err = testDB.QueryRowContext(ctx, "SELECT payload FROM jobs WHERE queue = ?", QueueProcessNotification).
		Scan(&payload)
	require.NoError(t, err)
...
func TestSQLiteScheduler_WorkerPoolProcessesNotifications(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		enqueueErr := scheduler.EnqueueProcessNotification(ctx, "1", data)
...
func TestSQLiteScheduler_JobDeadLetterAfterMaxAttempts(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		err = testDB.QueryRowContext(ctx, "SELECT status FROM jobs WHERE queue = ?", QueueProcessNotification).
			Scan(&status)
...
func TestSQLiteScheduler_JobsSurviveRestart(t *testing.T) {
	...
	var count int
	err := testDB.QueryRowContext(ctx, "SELECT COUNT(*) FROM jobs WHERE status = 'pending'").
		Scan(&count)
	require.NoError(t, err)
