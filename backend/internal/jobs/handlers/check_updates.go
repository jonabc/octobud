...
func (h *CheckUpdatesHandler) Handle(ctx context.Context) (bool, error) {
	...
	if settings.DismissedUntil != "" {
		...
		if parseErr == nil && time.Now().Before(dismissedUntil) {
			h.logger.Debug(
				"update check dismissed until",
				zap.String("until", settings.DismissedUntil),
			)
			return false, nil
		}
	}
	...
	if settings.LastCheckedAt != "" {
		lastChecked, parseErr := time.Parse(time.RFC3339, settings.LastCheckedAt)
		if parseErr == nil {
	...
	if info != nil {
		h.logger.Info("update available",
			zap.String("current", info.CurrentVersion),
			zap.String("latest", info.LatestVersion),
		)
		return true, nil
	}
