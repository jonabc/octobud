...
func (h *Handler) handleListAllTags(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to list tags")
		return
	}

	helpers.WriteJSON(w, http.StatusOK, listTagsResponse{Tags: tags})
}
...
func (h *Handler) handleCreateTag(w http.ResponseWriter, r *http.Request) {
	...
	t, err := h.tagSvc.CreateTag(ctx, userID, req.Name, req.Color, req.Description)
	if err != nil {
		h.logger.Error(
			"handleCreateTag - failed to create tag",
			zap.Error(errors.Join(ErrFailedToCreateTag, err)),
		)
		helpers.WriteError(w, http.StatusInternalServerError, "failed to create tag")
		return
	}

	// Convert db.Tag to TagResponse
	tagResp := models.TagFromDB(t)
	helpers.WriteJSON(w, http.StatusCreated, tagEnvelope{Tag: tagResp})
}
...
func (h *Handler) handleUpdateTag(w http.ResponseWriter, r *http.Request) {
	...
	if tagID == "" {
		helpers.WriteError(w, http.StatusBadRequest, "tag ID is required")
		return
	}
	...
	t, err := h.tagSvc.UpdateTag(ctx, userID, tagID, req.Name, req.Color, req.Description)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			helpers.WriteError(w, http.StatusNotFound, "tag not found")
			return
		}
		h.logger.Error(
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to update tag")
		return
	}

	// Convert db.Tag to TagResponse
	tagResp := models.TagFromDB(t)
	helpers.WriteJSON(w, http.StatusOK, tagEnvelope{Tag: tagResp})
}
...
func (h *Handler) handleDeleteTag(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to delete tag")
		return
	}
...
func (h *Handler) handleReorderTags(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.TagIDs) == 0 {
		helpers.WriteError(w, http.StatusBadRequest, "tagIDs is required")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to reorder tags")
		return
	}
	...
	helpers.WriteJSON(w, http.StatusOK, listTagsResponse{Tags: tagResponses})
}
