...
func (h *Handler) handleBulkOperation(w http.ResponseWriter, r *http.Request, op BulkOperation) {
	...
	if err != nil {
		...
		if errors.Is(err, notification.ErrNoNotificationIDs) {
			helpers.WriteError(w, http.StatusBadRequest, "no notification ids provided")
			return
		}
		helpers.WriteError(
			w,
	...
	helpers.WriteJSON(w, http.StatusOK, bulkNotificationsResponse{Count: int(count)})
}
...
func (h *Handler) handleBulkAssignTag(w http.ResponseWriter, r *http.Request) {
	...
	if req.TagID == "" {
		...
		helpers.WriteError(w, http.StatusBadRequest, "tagId is required")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to get tag")
		return
	}
	...
	if hasQuery {
		...
		if err != nil {
			...
			helpers.WriteError(w, http.StatusInternalServerError, "failed to list notifications")
			return
		}
	} else {
		...
		for _, githubID := range req.GithubIDs {
			notif, getErr := h.notifications.GetByGithubID(ctx, userID, githubID)
			if getErr != nil {
	...
	if len(notifications) == 0 {
		helpers.WriteJSON(w, http.StatusOK, bulkNotificationsResponse{Count: 0})
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to assign tag")
		return
	}

	helpers.WriteJSON(w, http.StatusOK, bulkNotificationsResponse{Count: count})
}
...
func (h *Handler) handleBulkRemoveTag(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to remove tag")
		return
	}
