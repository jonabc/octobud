...
func (h *Handler) handlePollForToken(w http.ResponseWriter, r *http.Request) {
	...
	if req.DeviceCode == "" {
		helpers.WriteError(w, http.StatusBadRequest, "Device code is required")
		return
	}
	...
	if err != nil {
		...
		case errors.Is(err, coregithub.ErrAuthorizationPending):
			helpers.WriteJSON(w, http.StatusOK, PollResponse{Status: "pending"})
			return
		...
		case errors.Is(err, coregithub.ErrExpiredToken):
			helpers.WriteJSON(w, http.StatusOK, PollResponse{Status: "expired"})
			return
		case errors.Is(err, coregithub.ErrAccessDenied):
			helpers.WriteJSON(w, http.StatusOK, PollResponse{Status: "denied"})
			return
		default:
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to save GitHub connection")
		return
	}
	...
	helpers.WriteJSON(w, http.StatusOK, PollResponse{
		Status:         "complete",
