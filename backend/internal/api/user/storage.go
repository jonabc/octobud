...
func (h *Handler) HandleGetStorageStats(w http.ResponseWriter, r *http.Request) {
	...
	if cleanupHandler == nil {
		helpers.WriteError(w, http.StatusInternalServerError, "Cleanup handler not configured")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to get storage stats")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to get retention settings")
		return
	}
...
func (h *Handler) HandleUpdateRetentionSettings(w http.ResponseWriter, r *http.Request) {
	...
	existingSettings, err := cleanupHandler.GetRetentionSettings(ctx, userID)
	if err != nil {
		// Log but continue - we'll just use empty lastCleanupAt
		h.logger.Warn("failed to get existing retention settings", zap.Error(err))
	}
	...
	if err := cleanupHandler.UpdateRetentionSettings(ctx, userID, settings); err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to update retention settings")
		return
	}
...
func (h *Handler) HandleRunCleanup(w http.ResponseWriter, r *http.Request) {
	...
	if req.RetentionDays <= 0 {
		helpers.WriteError(w, http.StatusBadRequest, "Retention days must be greater than 0")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to run cleanup")
		return
	}
...
func (h *Handler) HandleGetEligibleForCleanup(w http.ResponseWriter, r *http.Request) {
	...
	if settings == nil || settings.RetentionDays <= 0 {
		helpers.WriteJSON(w, http.StatusOK, map[string]int64{"count": 0})
		return
	}
	...
	helpers.WriteJSON(w, http.StatusOK, map[string]int64{"count": count})
}
