...
func (h *Handler) HandleUpdateSyncSettings(w http.ResponseWriter, r *http.Request) {
	...
	if req.InitialSyncDays != nil {
		if *req.InitialSyncDays < 1 {
			helpers.WriteError(w, http.StatusBadRequest, "initialSyncDays must be at least 1")
			return
		}
	...
	if req.InitialSyncMaxCount != nil {
		if *req.InitialSyncMaxCount < 1 {
			helpers.WriteError(w, http.StatusBadRequest, "initialSyncMaxCount must be at least 1")
			return
		}
		if *req.InitialSyncMaxCount > maxNotificationCount {
			helpers.WriteError(w, http.StatusBadRequest, "initialSyncMaxCount cannot exceed 100000")
			return
		}
	}
...
func (h *Handler) HandleGetSyncState(w http.ResponseWriter, r *http.Request) {
	if h.syncStateSvc == nil {
		helpers.WriteError(w, http.StatusServiceUnavailable, "Sync state service not available")
		return
	}
...
func (h *Handler) HandleSyncOlder(w http.ResponseWriter, r *http.Request) {
	...
	if req.Days < 1 {
		helpers.WriteError(w, http.StatusBadRequest, "days must be at least 1")
		return
	}
	if req.Days > maxSyncDays {
		helpers.WriteError(w, http.StatusBadRequest, "days cannot exceed 3650 (10 years)")
		return
	}

	if req.MaxCount != nil {
		if *req.MaxCount < 1 {
			helpers.WriteError(w, http.StatusBadRequest, "maxCount must be at least 1")
			return
		}
		if *req.MaxCount > maxNotificationCount {
			helpers.WriteError(w, http.StatusBadRequest, "maxCount cannot exceed 100000")
			return
		}
	}
	...
	if h.scheduler == nil {
		helpers.WriteError(w, http.StatusServiceUnavailable, "Job queue not available")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "Failed to queue sync job")
		return
	}
