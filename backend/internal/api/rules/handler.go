...
func (h *Handler) handleListRules(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		helpers.WriteError(w, http.StatusInternalServerError, "failed to load rules")
		return
	}
	...
	helpers.WriteJSON(w, http.StatusOK, listRulesResponse{Rules: response})
}
...
func (h *Handler) handleGetRule(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, rulescore.ErrRuleNotFound) {
			helpers.WriteError(w, http.StatusNotFound, "rule not found")
			return
		}
		helpers.WriteError(w, http.StatusInternalServerError, "failed to get rule")
		return
	}

	helpers.WriteJSON(w, http.StatusOK, ruleEnvelope{Rule: rule})
}
...
func (h *Handler) handleCreateRule(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to create rule")
		return
	}
	...
	helpers.WriteJSON(w, http.StatusCreated, ruleEnvelope{Rule: createdRule})
}
...
func (h *Handler) handleDeleteRule(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.ruleSvc.DeleteRule(ctx, userID, ruleID); err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to delete rule")
		return
	}
...
func (h *Handler) handleReorderRules(w http.ResponseWriter, r *http.Request) {
	...
	if len(req.RuleIDs) == 0 {
		helpers.WriteError(w, http.StatusBadRequest, "ruleIDs cannot be empty")
		return
	}
	...
	if err != nil {
		...
		helpers.WriteError(w, http.StatusInternalServerError, "failed to reorder rules")
		return
	}
