...
// Package xcrypto provides a fallback implementation of the Keychain interface for non-macOS platforms.
package xcrypto
...
// StoreToken is not supported on non-macOS platforms. Tokens
// should be stored using Encryptor + SQLite instead.
...
// GetToken is not supported on non-macOS platforms. Tokens
// should be retrieved using Encryptor + SQLite instead.
func (k *keychainFallback) GetToken(service, account string) (string, error) {
	return "",
		fmt.Errorf("keychain is not available on this platform, use SQLite encryption instead")
}
