...
func (k *keychainImpl) StoreToken(service, account, token string) error {
	...
	if err := k.DeleteToken(service, account); err != nil {
		...
		if strings.Contains(err.Error(), "-25244") {
			return fmt.Errorf(
				"keychain item exists but cannot be updated due to ownership conflict. "+
					"Please manually delete it: Open Keychain Access.app, search for '%s' with account '%s', "+
	...
	if err != nil {
		...
		if strings.Contains(errStr, "-25299") || errStr == "Item already exists" {
			return fmt.Errorf(
				"failed to store token: item already exists (this should not happen). Please try again",
			)
		}
...
func (k *keychainImpl) DeleteToken(service, account string) error {
	...
	if err != nil {
		errStr := err.Error()
		// It's OK if the item doesn't exist - check for various "not found" error formats
		if err.Error() == "Item not found" ||
			strings.Contains(errStr, "-25300") ||
			strings.Contains(errStr, "could not be found") ||
			strings.Contains(errStr, "itemNotFoundErr") {
			return nil
		}
		...
		if errStr == "An invalid attempt to change the owner of an item. (-25244)" ||
			...
			return fmt.Errorf(
				"cannot delete keychain item due to ownership (created by different app version): %w. "+
					"Please manually delete it from Keychain Access.app",
				err,
			)
		}
